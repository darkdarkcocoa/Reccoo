...
- **UI 문구**: 현재 앱 UI와 마스코트 대사는 **한국어 전용**이며, 딱딱한 격식체가 아니라 **친근하고 다정한 톤**입니다. 문구를 추가하거나 수정할 때 이 목소리를 유지해 주세요.
...
Key rules: keep the flat no-framework architecture, no new NuGet deps without discussion, reuse the `App.xaml` design tokens (no hard-coded colors), and match the friendly, warm Korean voice in all UI strings (the UI is currently Korean-only).

## 🌏 번역 / Localization

영어 UI 지원은 아직 없습니다. 관심이 있으시면 이슈로 알려 주세요 — 언제나 환영입니다!
