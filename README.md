...
레코는 PC에서 재생 중인 소리를 그대로 녹음해 주는 작은 레코더입니다. 스트리밍 음악, 게임 사운드, 라이브 방송 등 스피커로 출력되는 소리라면 무엇이든 버튼 하나로 녹음해 WAV 또는 MP3로 저장할 수 있습니다. 마이크를 거치지 않기 때문에 주변 잡음 없이 깨끗하게 녹음되며, 녹음하는 동안에는 픽셀 마스코트 "레코"가 곁에서 말을 걸어 줍니다. 🐤
...
>
> .NET 런타임이 포함된 single-file 실행파일입니다. 처음 실행할 때 SmartScreen이 "알 수 없는 앱" 경고를 표시할 수 있는데, 아직 코드 서명 인증서를 적용하지 않았기 때문입니다. **"추가 정보" → "실행"**을 누르면 정상적으로 실행됩니다.
...
- **3초 카운트다운** — 시작 버튼을 누르면 3초를 세어 주어 녹음 타이밍을 맞추기 좋습니다
- **일시정지 / 재개** — 광고처럼 원치 않는 구간은 잠시 멈췄다가 이어서 녹음할 수 있습니다
- **WAV / MP3 선택** — 원본 그대로 보관하려면 WAV, 용량을 아끼려면 MP3 (LO / MED / HI 품질 선택)

**👀 보는 즐거움**

- **라이브 파형** — 재생 중인 소리가 56개의 픽셀 바로 실시간 표시됩니다 (mint → gold → coral 그라데이션)
- **입력 레벨 미터** — 18칸 픽셀 미터로 소리 크기를 한눈에 확인할 수 있습니다
- **마스코트 "레코"** — 소리에 맞춰 움직이고, 녹음 상태에 따라 표정이 바뀌며, 가끔 말을 걸어 옵니다
- **다크 테마** — 타이틀바의 토글 버튼으로 밝은/어두운 테마를 전환합니다

**📼 녹음 후**

- **보관함** — 녹음본이 카세트 테이프 모양의 카드로 정리됩니다. 재생 길이 표시와 카드에서 바로 이름 변경을 지원합니다
- **미니 플레이어** — 앱 안에서 바로 재생하고, 진행바로 원하는 위치를 탐색할 수 있습니다
- **드래그로 내보내기** — 카드를 끌어 폴더나 메신저 창에 놓으면 파일이 복사됩니다
- **휴지통 삭제** — 삭제한 파일은 휴지통으로 이동하므로 실수해도 복구할 수 있습니다
- **키보드 단축키** — 마우스 없이도 녹음을 시작하고 멈출 수 있습니다
